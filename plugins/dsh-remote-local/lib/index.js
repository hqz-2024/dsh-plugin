...
const NON_ADMIN_DENY = new Set([
	...
	// The /permission command (and every other client command: /export, /plan,
 ...
 */
/** Module-scope no-op; apply() installs the file-backed `diag` that shadows this inside its closure. */
const diag = () => {};
...
function evaluateRoleGate(pathname, httpMethod, body, role, user, mapResolver, mappedUsers) {
	...
	try {
		...
		diag("evaluate PARSE-FAIL path=" + pathname + " bodyBytes=" + body.length);
		return { allowed: false, body, replayed: true };
	}
	diag("evaluate path=" + pathname + " type=" + String(envelope?.type) + " method=" + String(envelope?.method));
	if (envelope?.type !== "client-request" || typeof envelope.method !== "string") {
		return { allowed: true, body, replayed: true };
	}
	const method = envelope.method;
	const methodDot = method.replace(/\//g, ".");
	if (NON_ADMIN_DENY.has(methodDot)) return { allowed: false, body, replayed: true };
	if (role === "guest" && GUEST_DENY.has(methodDot)) return { allowed: false, body, replayed: true };
	// Fail closed for a non-admin account without a roleMap entry: no session
	// surface at all until an admin maps the account (an unmapped session.list
	// would return every session, since no scopeUser would be stamped). The
	// wrapper enforces the same rule on the runtime path.
	if (role !== "admin" && typeof user === "string" && mappedUsers !== undefined && !mappedUsers.has(user)
		&& methodDot.startsWith("session.")) {
		return { allowed: false, body, replayed: true };
	}
	// Role-map injection (local fork): rewrite session.create payloads for
	// mapped accounts before replay. mapResolver returns a Buffer to replay,
	// or null to pass the original body through untouched.
	if (typeof user === "string" && typeof mapResolver === "function") {
		const rewritten = mapResolver(user, method, envelope);
		if (rewritten !== null) return { allowed: true, body: rewritten, replayed: true };
	}
	return { allowed: true, body, replayed: true };
}
...
// ── per-account session ownership service (local fork) ─────────────────
...
if (ownersCache === null) { try { ownersCache = JSON.parse(readFileSync(OWNERS_PATH, "utf8")); } catch { ownersCache = {}; } }
	
	
return ownersCache[sessionId] ?? null;
	
};
	
const setOwner = (sessionId, username) => {
	
	
ownerOf(sessionId);
	
	
if (ownersCache[sessionId] === username) return;
	
	
ownersCache[sessionId] = username;
	
	
try { writeFileSync(OWNERS_PATH, JSON.stringify(ownersCache, null, 2)); } catch { /* keep in memory */ }
	
};
	
// ── per-item hide state (admin-only visibility toggle, local fork) ──────
// Hidden workspaces/sessions stay visible to admin (admin never receives
// scopeUser) and are filtered from non-admin surfaces: sessions through this
// ownership reader, workspace rows through the browser half's DOM hiding.
const HIDDEN_PATH = join(dshHomePath(), "auth", "hidden-items.json");
let hiddenCache = null;
const hiddenState = () => {
  if (hiddenCache === null) {
    try { hiddenCache = JSON.parse(readFileSync(HIDDEN_PATH, "utf8")); } catch { hiddenCache = null; }
  }
  if (hiddenCache === null || typeof hiddenCache !== "object") hiddenCache = {};
  if (!Array.isArray(hiddenCache.workspaces)) hiddenCache.workspaces = [];
  if (!Array.isArray(hiddenCache.sessions)) hiddenCache.sessions = [];
  return hiddenCache;
};
const saveHidden = () => {
  try { writeFileSync(HIDDEN_PATH, JSON.stringify(hiddenState(), null, 2)); } catch { /* keep in memory */ }
};
const isHiddenSession = (sessionId) => hiddenState().sessions.indexOf(sessionId) !== -1;
const hiddenWorkspacePaths = () => {
  try {
    const byId = new Map((ctx.get("workspaceRegistry")?.list?.() ?? []).map((w) => [w.id, w.path]));
    return hiddenState().workspaces.map((id) => byId.get(id)).filter((p) => typeof p === "string" && p.length > 0);
  } catch { return []; }
};
const normPathLower = (p) => String(p ?? "").replace(/\\/g, "/").replace(/\/+$/, "").toLowerCase();
const underHiddenWorkspace = (cwd) => {
  if (typeof cwd !== "string" || cwd.length === 0) return false;
  const c = normPathLower(cwd);
  for (const p of hiddenWorkspacePaths()) {
    const r = normPathLower(p);
    if (r && (c === r || c.indexOf(r + "/") === 0)) return true;
  }
  return false;
};
// Strict per-account visibility: only sessions OWNED by `user` are visible;
// unowned legacy sessions are invisible to non-admin accounts until claimed
// by use (prompt/rename/attachment). Admin never receives scopeUser and
// stays unfiltered. Hidden items are additionally invisible to non-admin.
const isVisible = (user, sessionId, cwd) => {
  const o = ownerOf(sessionId);
  if (o === null || o !== user) return false;
  if (isHiddenSession(sessionId)) return false;
  if (underHiddenWorkspace(cwd)) return false;
  return true;
};
	
// Provide the ownership reader at the ROOT scope: the official
// session-controller resolves it with `ctx.get('sessionOwnership')` from the
// root global store, so a provide inside `ctx.effect(...)` (a child scope) was
...
ctx.effect(() => () => { disposeOwnership(); }, "dsh-remote: sessionOwnership disposal");
	...
	// (write boundary = the session's immutable cwd, i.e. the finance-ws
	// folder) with approval `never`, so an escalation to `danger-full-access`
	// is rejected deterministically — a finance account cannot widen itself.
	...
	const confineSession = (session) => {
		...
		try {
			...
			if (state === null || state.preset !== "finance-confined") {
				session.append("permission/preset", { preset: "finance-confined" });
			}
			diag("confined session " + session.id + " workspace-write/never preset=" + String(session?.header?.agentPreset ?? "") + " cwd=" + String(session?.header?.cwd ?? ""));
		} catch (err) {
			diag("confine failed session=" + session.id + ": " + String(err?.message ?? err));
		}
	};
	// Arm the confinement as soon as the sessions service exists. This plugin
	// may apply before `sessions` is active (its own injection is only
	// `webServer`), so read it through a deferred injection instead of a
	// one-shot `ctx.get`: the callback fires when the service appears and
	// pins every session from then on — including the ones already loaded.
	const armConfinement = (scope) => {
		if (scope?.sessions?.list) {
			scope.on("session/created", confineSession);
			for (const session of scope.sessions.list()) confineSession(session);
			diag("confinement armed on sessions service");
		} else {
			diag("confinement not armed: injected scope lacks sessions");
		}
	};
	try {
		ctx.inject(["sessions"], armConfinement);
	} catch (err) {
		diag("ctx.inject sessions failed: " + String(err?.message ?? err));
	}
	// ── per-role file-pane confinement (P8, 2026-09-02) ─────────────────────
	// The folder-tree-sh pane whitelists ALL registered workspace roots, which
	...
	const pathUnderRoot = (rawPath, root) => {
		...
		const rootN = normFtreePath(root);
		const candN = normFtreePath(rawPath);
		if (candN !== rootN && !candN.startsWith(rootN + "/")) return false;
		// Lexical containment passed; realpath the deepest existing ancestor
		// to close `..` and symlink/junction escapes.
		let probe = rawPath;
		let real = null;
		for (let i = 0; i < 64; i++) {
			try { real = realpathSync(probe); break; } catch { /* ancestor missing — ascend */ }
			const idx = Math.max(probe.lastIndexOf("/"), probe.lastIndexOf("\\"));
			if (idx <= 0) break;
			probe = probe.slice(0, idx);
		}
		if (real === null) return true; // brand-new path under the root: lexical check is the boundary
		const realN = normFtreePath(real);
		return realN === rootN || realN.startsWith(rootN + "/");
	};
	...
	// Role-map injection (local fork): per-account rewrite of session.create.
	// `preset` sets the agent preset id; `workspace` (matched by workspace
	...
	const roleGate = async (req, role, username) => {
		...
		const body = req.method === "POST"
			...
			: null;
		return evaluateRoleGate(pathname, req.method, body, role, username, mapResolver);
	};
	...
	*/
	...
	const wrapHttp = (handler) => {
		...
		const gated = async (req, res) => {
			...
			// channel so a non-admin cannot query it directly either.
			if (gatePath.startsWith("/usage-stats") && verdict.user.role !== "admin") {
				denyJson(outRes, 403, "admin only");
				return;
			}
			// The workspace file pane is per-account confined: a mapped account
			// may only list/read/write/rename inside its OWN mapped workspace
			// (e.g. finance-ws), never the other registered workspaces. The
			// meta/token routes carry no path and pass. Guests may not use the
			// pane at all. The authenticated role rides an internal header so
			// the pane host can relax its own workspace whitelist for admin.
			if (gatePath.startsWith("/dsh-ftree")) {
				req.headers["x-dsh-role"] = verdict.user.role;
				if (verdict.user.role === "guest") {
					diag("deny ftree for guest " + verdict.user.username);
					denyJson(outRes, 403, "需要升级权限才能使用该功能");
					return;
				}
				if (verdict.user.role !== "admin") {
					...
					if (!isMeta && mappedRoots.length > 0) {
						...
						try {
							...
							if (qDir) candidates.push(qDir);
						} catch { /* unparsable query */ }
						if (req.method === "POST" && !isQueryPathPost) {
							const b = await readBodyBounded(req, 64 * 1024);
							if (b === null) { denyJson(outRes, 403, "request too large"); return; }
							try {
								const parsed = JSON.parse(b.toString("utf8"));
								for (const k of ["path", "srcPath", "destDir", "target"]) {
							...
							if (candidates.length > 0 && candidates.every((p) => mappedRoots.some((root) => pathUnderRoot(p, root)))) {
								...
								return handler(replayable(req, b), outRes);
							}
						} else {
					...
					if (!allowed) {
						diag("deny ftree " + gatePath + " user=" + verdict.user.username);
						denyJson(outRes, 403, "超出本账号工作区范围");
						return;
					}
					// Query-path POSTs carry their payload as the body: pass them
					// straight through so no later gate consumes the stream.
					if (isQueryPathPost) return handler(req, outRes);
				}
			}
			const isApiPost = req.method === "POST" && gatePath.startsWith("/api/");
			if (cfg.enforceRoles && verdict.user.role !== "admin") {
				if (isApiPost) {
					...
					if (body !== null) {
						let env = null;
						try { env = JSON.parse(body.toString("utf8")); } catch { /* unreadable */ }
						if (env && env.type === "client-request" && typeof env.method === "string") {
							...
							// session.list would return every session, since no
							// scopeUser would be stamped).
							if (mapping === undefined && methodDot.startsWith("session.")) {
								diag("deny " + methodDot + ": unmapped account " + verdict.user.username);
							...
							const rewritten = mapResolver(verdict.user.username, methodDot, env);
							if (rewritten !== null) {
								diag("wrapper injected for " + verdict.user.username);
								return handler(replayable(req, rewritten), outRes);
							}
							if (mappedWs !== null) {
								const SESSION_METHODS = new Set(["session.prompt","session.page","session.rename","session.attachment","session.updateQueue","session.cancel","session.delete","session.fork","session.selectModel"]);
								const rawArgs = argsOf(env);
								const sessionId = rawArgs?.request?.sessionId ?? rawArgs?.sessionId;
								if (SESSION_METHODS.has(methodDot) && typeof sessionId === "string") {
									const sidOwner = ownerOf(sessionId);
									if (sidOwner !== null && sidOwner !== verdict.user.username) {
										diag("deny session op " + methodDot + " session=" + sessionId + " owned by " + sidOwner);
										denyRpcEnvelope(outRes, body, "session belongs to another account");
										return;
									}
									// Hard boundary: a mapped account may only operate on
									// sessions inside its OWN mapped workspace. This also
									// closes the earlier leak where a user who opened a
									// foreign workspace's unowned session claimed it.
									const sessionCwd = ctx.get("sessions")?.get?.(sessionId)?.header?.cwd ?? null;
									if (isHiddenSession(sessionId) || underHiddenWorkspace(sessionCwd)) {
										diag("deny session op " + methodDot + " session=" + sessionId + " hidden");
									...
									if (sidOwner === null && (methodDot === "session.prompt" || methodDot === "session.rename" || methodDot === "session.attachment")) {
										setOwner(sessionId, verdict.user.username);
										diag("claimed session " + sessionId + " for " + verdict.user.username);
									}
								}
							...
							function argsOf(env) { const p = env?.payload ?? {}; return p.args ?? {}; }
							function teeResponse(res, onParsed) {
								const chunks = [];
								const origEnd = res.end.bind(res);
								const origWrite = res.write.bind(res);
								res.write = (chunk, enc, cb) => { if (chunk) chunks.push(Buffer.from(chunk)); if (typeof enc === "function") enc(); else if (typeof cb === "function") cb(); return true; };
								res.end = (chunk, enc, cb) => {
									if (chunk) chunks.push(Buffer.from(chunk));
									let out = Buffer.concat(chunks).toString("utf8");
									try {
										const parsed = JSON.parse(out);
											diag("tee parsed ok=" + String(parsed?.result?.ok) + " type=" + String(parsed?.type) + " valueKeys=" + String(Object.keys(parsed?.result?.value ?? {}).join(",")));
										onParsed(parsed);
										out = JSON.stringify(parsed);
									} catch { /* not JSON: pass through */ }
									diag("tee parse-fail raw=" + out.slice(0, 160));
									res.write = origWrite; res.end = origEnd;
									if (!res.headersSent) { try { res.setHeader("content-length", Buffer.byteLength(out)); } catch { /* headers already flushed */ } }
									return origEnd(out);
									if (typeof enc === "function") enc(); else if (typeof cb === "function") cb();
								};
								return res;
							}
							function filterSessionListResponse(res, keep) {
								return teeResponse(res, (parsed) => {
									if (!(parsed?.result?.ok === true) || !Array.isArray(parsed.result.value?.items)) return;
									const before = parsed.result.value.items.length;
									parsed.result.value.items = parsed.result.value.items.filter(keep);
									diag("session.list filtered " + before + " -> " + parsed.result.value.items.length + " items");
								});
							}
						}
						return handler(replayable(req, body), outRes);
					...
					const gate = await roleGate(req, verdict.user.role, verdict.user.username);
					if (!gate.allowed) { denyRpcEnvelope(outRes, gate.body, "forbidden for role " + verdict.user.role); return; }
	...
	const handleMe = async (req, res) => {
		...
		const verdict = requireAuth(req);
		let workspaces = null;
		let hiddenWorkspaceTitles = [];
		let hiddenSessionTitles = [];
		if (verdict.ok) {
			try {
		...
		json(res, 200, {
			...
			workspaces,
			hiddenWorkspaceTitles,
			hiddenSessionTitles
		});
	};

	const handleHide = async (req, res) => {
		...
		const body = await readBodyBounded(req, 64 * 1024);
		let input = null;
		if (body !== null) { try { input = JSON.parse(body.toString("utf8")); } catch { input = null; } }
		const kind = input?.kind;
		const title = typeof input?.title === "string" ? input.title.trim() : "";
		const hidden = input?.hidden === true;
		if (kind !== "workspace" && kind !== "session") { denyJson(res, 400, "invalid kind"); return; }
		if (!title) { denyJson(res, 400, "missing title"); return; }
		diag("hide req kind=" + kind + " title=" + title + " hidden=" + hidden + " user=" + verdict.user.username);
		let id = null;
		if (kind === "workspace") {
