...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── role-based UI hiding (local fork) ─────────────────────────────────────
		// Non-admin mapped users lose the workspace/preset switching chrome; enforcement is
		// server-side (session/workspace gates), these rules only clean the UI. Runs on every
		// /auth/me poll, so a role change applies within 15s without a reload.
		function applyRoleCss(me) {
			if (!me || typeof me !== "object") return;
			const mapped = me.authenticated === true && me.role !== "admin" && typeof me.role === "string";
			const root = document.documentElement;
			const existing = document.getElementById("dsh-role-css");
			if (!mapped) {
				root.removeAttribute("data-dsh-role");
				if (existing) existing.remove();
				return;
			}
			root.setAttribute("data-dsh-role", me.role);
			if (existing) return;
			const style = document.createElement("style");
			style.id = "dsh-role-css";
			style.textContent = [
				"button[aria-label=\u0022添加工作区\u0022], button[aria-label=\u0022Add workspace\u0022] { display: none !important; }",
				"button[aria-label=\u0022选择工作区\u0022], button[aria-label=\u0022Choose workspace\u0022] { display: none !important; }",
				"[title=\u0022即将开始的这个会话所用的 Agent 预设\u0022], [title=\u0022Agent preset for the session you are about to start\u0022] { display: none !important; }",
				"[title=\u0022本会话运行的 Agent 预设，开始时即固定\u0022] { display: none !important; }"
			].join("\n");
			document.head.appendChild(style);
		}
		// ── per-item hide state (local fork) ───────────────────────────────
		let currentRole = null;
		let hiddenWorkspaceTitles = new Set();
		let hiddenSessionTitles = new Set();

		// Parse a row "…" anchor aria-label into {kind,title}; null if not a
		// workspace/session row menu trigger.
		function parseMenuAnchor(label) {
			if (!label) return null;
			let m = label.match(/^工作区“(.+)”的操作$/);
			if (m) return { kind: "workspace", title: m[1] };
			m = label.match(/^Workspace "(.+)" actions$/);
			if (m) return { kind: "workspace", title: m[1] };
			m = label.match(/^会话“(.+)”的操作$/);
			if (m) return { kind: "session", title: m[1] };
			m = label.match(/^Session "(.+)" actions$/);
			if (m) return { kind: "session", title: m[1] };
			return null;
		}

		// Hide sidebar workspace rows whose title is not allowed for this account,
		// plus admin-hidden workspaces and sessions for non-admin (titles come from
		// /auth/me; workspace rows carry aria-label = title, session rows are found
		// through their "…" button's aria-label).
		function hideForeignWorkspaces(me) {
			currentRole = (me && typeof me.role === "string") ? me.role : null;
			hiddenWorkspaceTitles = new Set(Array.isArray(me?.hiddenWorkspaceTitles) ? me.hiddenWorkspaceTitles : []);
			hiddenSessionTitles = new Set(Array.isArray(me?.hiddenSessionTitles) ? me.hiddenSessionTitles : []);
			const rows = Array.isArray(me?.workspaces) ? me.workspaces : [];
			const foreign = new Set(rows.filter((w) => w.allowed === false).map((w) => w.title));
			// Hidden items are filtered from non-admin only; admin sees all.
			const hiddenWs = currentRole === "admin" ? new Set() : new Set(hiddenWorkspaceTitles);
			let scheduled = false;
			const apply = () => {
				if (scheduled) return;
				scheduled = true;
				queueMicrotask(() => {
					scheduled = false;
					// Find rows through their "…" trigger's aria-label, then hide the
					// enclosing treeitem row (workspace and session rows alike).
					for (const el of document.querySelectorAll("button[aria-label]")) {
						const parsed = parseMenuAnchor(el.getAttribute("aria-label") || "");
						if (!parsed) continue;
						const row = el.closest('[role="treeitem"]');
						if (!row) continue;
						const shouldHide = parsed.kind === "workspace"
							? (foreign.has(parsed.title) || hiddenWs.has(parsed.title))
							: (currentRole !== "admin" && hiddenSessionTitles.has(parsed.title));
						if (shouldHide) {
							row.style.display = "none";
							row.setAttribute("data-dsh-hidden", "1");
						} else if (row.getAttribute("data-dsh-hidden") === "1") {
							row.removeAttribute("data-dsh-hidden");
							row.style.display = "";
						}
					}
			...
			apply();
			if (!window.__dshWsObs) {
				window.__dshWsObs = new MutationObserver(apply);
				window.__dshWsObs.observe(document.body, { childList: true, subtree: true });
			}
		}

		// Inject an admin-only 隐藏/取消隐藏 item into the workspace/session
		// three-dot menus. Rows carry no ids, so targets resolve by title
		// (workspace titles are unique; a duplicate session title resolves to the
		// first match host-side).
		function showToast(message) {
			try {
				const el = document.createElement("div");
				Object.assign(el.style, {
					position: "fixed", top: "16px", right: "16px", zIndex: "2147483001",
					padding: "8px 14px", borderRadius: "8px", fontSize: "13px",
					background: "var(--dsw-specific-menu, #161b22)", color: "var(--dsw-alias-label-primary, #e6edf3)",
					border: "1px solid var(--dsw-alias-border-l2, #30363d)", boxShadow: "0 6px 20px rgba(0,0,0,.3)",
					maxWidth: "360px"
				});
		...
		function installHideMenuButton() {
			if (window.__dshHideMenuInstalled) return;
			window.__dshHideMenuInstalled = true;
			...
			document.addEventListener("click", (event) => {
				...
				if (!btn) return;
				const parsed = parseMenuAnchor(btn.getAttribute("aria-label") || "");
				if (parsed) pendingTarget = parsed;
			}, true);
			const inject = (menu) => {
				if (menu.__dshHideInjected) return;
				const target = pendingTarget;
				if (!target) return;
				menu.__dshHideInjected = true;
				pendingTarget = null;
				const set = target.kind === "workspace" ? hiddenWorkspaceTitles : hiddenSessionTitles;
				const isHidden = set.has(target.title);
				...
				Object.assign(button.style, {
					...
					color: "inherit", fontSize: "13px", cursor: "pointer", textAlign: "left"
				});
				button.textContent = isHidden ? "取消隐藏" : "隐藏";
				button.addEventListener("click", (event) => {
					event.stopPropagation();
					fetchJson("/auth/hide", { kind: target.kind, title: target.title, hidden: !isHidden }).then((result) => {
						if (result && result.ok) {
							if (isHidden) set.delete(target.title); else set.add(target.title);
							showToast(isHidden ? "已取消隐藏「" + target.title + "」" : "已隐藏「" + target.title + "」");
							if (target.kind === "workspace") {
								hideForeignWorkspaces({ workspaces: [], hiddenWorkspaceTitles: Array.from(hiddenWorkspaceTitles), hiddenSessionTitles: Array.from(hiddenSessionTitles), role: currentRole });
							}
						} else {
							showToast("隐藏失败：" + ((result && result.error) || "未知错误"));
						}
					}).catch((e) => showToast("隐藏请求失败：" + String((e && e.message) || e)));
					try { document.body.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true })); } catch (e) { /* menu stays open */ }
				});
				const viewport = menu.querySelector('[role="presentation"]') || menu;
				viewport.appendChild(button);
			};
			const observer = new MutationObserver(() => {
				for (const menu of document.querySelectorAll('[role="menu"]')) inject(menu);
			});
		...
		function hideSettingsNav() {
			...
			const apply = () => {
				...
				queueMicrotask(() => {
					scheduled = false;
					const mapped = document.documentElement.getAttribute("data-dsh-role") !== null;
					for (const el of document.querySelectorAll("button")) {
						if (el.textContent && el.textContent.trim() === "Agent 预设") {
							el.style.display = mapped ? "none" : "";
						}
					}
				});
			};
			apply();
			if (!window.__dshPresetNavObs) {
				window.__dshPresetNavObs = new MutationObserver(apply);
				window.__dshPresetNavObs.observe(document.body, { childList: true, subtree: true });
			}
		}
		...
		function AuthGate() {
			...
			const check = react.useCallback(() => {
				fetchJson("/auth/me").then((result) => {
					...
					setMe(result); applyRoleCss(result); hideSettingsNav(); hideForeignWorkspaces(result); installHideMenuButton();
				});
			}, []);
			react.useEffect(() => {
				check();
			...
			const downloadExecutorCmd = async () => {
				try {
					...
					const content = "@echo off\r\n"
						...
						// The credential is passed explicitly: a copy built without a deployment
						// carries nothing, and the script is what makes this machine known to the
						// server. --ca is explicit for the same reason even though the program
						// also looks for caddy-root.crt beside itself.
