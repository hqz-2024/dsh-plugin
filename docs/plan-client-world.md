...
- **用户电脑**：一个 executor（**已完成**：Node + 本地配置页 + 开机自启，并已打包成单文件 exe 与 `node-pty` 同装一包分发；见 `plan-client-world-progress.md` §7-D）。启动后打开本地配置页 → 登录 → 选/绑定工作区。
...
> **落地情况（2026-09-16）**：两步都已走完，但**没有走 Electron/Tauri** —— 外壳用的是 **Node SEA 单文件 exe**，因为执行器是无界面的，它的界面本来就是那个本地配置页，所以 exe 只需要买三件事：客户端不用装 Node、可以双击、以及未来放托盘和更新通道的位置。分发形态是 `dsh-executor.zip`（exe + 裁剪过的 `node-pty`，约 32 MB），下载端点流式分发。**托盘没做**（计划里是可选）；**更新通道**做到"服务端能点名哪台机器落后于当前分发包"（`plan-client-world-progress.md` §7-D-4），**没有做自动下载与自替换** —— 那是 Electron 那类外壳才划算的部分，当前装机量下人工重下更省事。
...
| 5 | executor 的分发与更新（复用下载端点；exe 阶段还要自动更新通道） | P1 | **已完成分发**：`/dsh-subprocess-dispatch/dsh-executor.zip`（流式 + Range 续传）与设置页卡片上的 `/auth/executor-pack`。**更新通道只做到"点名落后"** —— 服务端比对分发包与客户端自报的构建时间并在日志里点名，不自动下载/自替换（§2.5 落地情况） |
