...
| **机器工具** | agent 直接操作客户端电脑（不绑定、不共享） | ✅ `machine_list` / `machine_run` 注册成功、按名字调用通过；断言落在"那台机器上的进程自报的事实"上（含超时不留残进程）（§7-D-10） |
| **exe 缺陷** | 打包形态下每结束一个终端就多出一个执行器 | ✅ **已复现并修复**（node-pty 的 `fork` 跑的是 `process.execPath`）；进程监视器修前 `n=2/3`、修后整轮 `n=1`，`terminal-python-repl` 由 ❌ 转 ✅（§7-D-11） |
...
#### D-2. 配置页（exe 唯一的用户界面）的自动化检查

页面是**模板字符串生成**的，`node --check` 看不见它的运行期错误 —— 这一类 bug 已经发生过两次（模板吃掉了 `\n`；页面调用了服务器侧才有的 helper）。所以用真浏览器跑一遍：

```powershell
# 先用某个端口起 exe（启动脚本形态或未注册形态都行），然后：
node "$env:USERPROFILE\.dsh\check-executor-enroll.mjs" --port 38479 --workspace 宝单科技资料
# 未注册形态加：--server http://127.0.0.1:3084 --user <账号> --password <密码>
```

它验的是：页面加载无脚本报错 → 登录换到凭据（或"启动脚本形态无需登录即列出工作区"）→ 工作区列表渲染 → **可见路径按服务器共享规则预填** → 点「绑定」看到「绑定成功」→ 绑定落到 `/status` 的 `heldShares`。

**两种形态都实测过**：

| 形态 | 结果 |
|---|---|
| 未注册（用账号密码登录，3084 的 `probe-admin`） | **8/8**，`宝单科技资料 → 绑定成功`，`heldShares=1` |
| 启动脚本（带 token，无密码） | **9/9**，`宝单科技资料 → 绑定成功` |

**判读经验（这条容易误判，必须记下）**：同一账号**已经活着的**绑定会让后续绑定**合法地被拒绝**，返回「绑定失败」，**看起来像"绑定按钮坏了"**。实测三次假失败全是这个原因 —— 每次检查都会真的绑上一个工作区，接着再跑一次就撞上自己上一次留下的绑定。**所以复跑这个检查前，先把该账号的绑定清掉**（`POST /unbind {workspaceId}`），否则得到的是"冲突被正确拒绝"而不是"绑定坏了"。

**两种拒绝都是对的，而且都带原因**（直连端点复现，响应都在 0.3 秒内）：

| 用例 | 服务器回答 |
|---|---|
| `.dsh`（没有共享建议，可见路径预填为空） | **400** `invalid-visible-path`：「the workspace must be given as an absolute path on the client machine (C:\work or \\server\share); got ""」 |
| 已被**同一账号另一条活绑定**占着的工作区 | 拒绝（页面显示「绑定失败」） |
...
#### D-7. Web UI 里的绑定状态与实时绑定/解绑（2026-09-16，用户需求变更）

**用户的原话**：「目前共享工作区的操作太麻烦了。而且在 web ui 里面没有体现，左侧工作区里面应该要有工作区的绑定状态提示…而且应该在 web ui 里面应该可以实时配置工作区绑定和工作区解绑…能不能绑定时候的，本机可见路径自动生成。本机暂存目录也自动生成。」

**先查清了"能不能插"**（这一步决定做法，不能拍脑袋）：

| 想改的地方 | 结论 |
|---|---|
| 左侧工作区列表 | 由引擎 `WorkspaceBrowser`（1379 行）渲染，占 `sidebar.workspaces` 这个 **`single` slot，没有子插槽** → 部署插不进去；fork 它要连 6 个内部模块和 2 个 CSS 一起搬 |
| 渲染出来的行 | `div.x8saxG_projectRow`，**哈希类名、无 id / data 属性** → DOM 注入属脆弱写法，不采纳 |
| **文件树栏** | **这个 Web UI 里没有**（43 个 client 包 + 40 行 bundle 全查过，只有 `@` 提及面板与"产出文件"） |
| 可用的加法位 | `conversation.session.header.actions`（**list**，session scope）、`conversation.input.left`（list）、`shell.overlay`（list，文档明说徽标/状态药丸放这里）、`settings.section`（list） |

**用户选定：会话标题栏**（`conversation.session.header.actions`）。它是 session scope，**天然知道当前会话属于哪个工作区**，状态放在那里是准确的。

**实现**：

| 层 | 内容 |
|---|---|
| host | 新增 `/client-web` 前缀（**留在门禁内**，调用者是已登录的人，session cookie 正是该有的证据）：`GET /state?cwd=…`、`POST /bind`、`POST /unbind` |
| 工作区怎么认 | 用**会话自己的 `cwd`** 反查，而不是让前端先查 workspace id —— `cwd` 就是分派器决定"命令跑在哪"用的那个事实，所以徽标描述的是**真实路由**，不是另一份可能不一致的猜测 |
| 自动生成 | 可见路径 = 按 `visiblePathHints` 把服务器目录翻成共享路径；暂存目录 = `%USERPROFILE%\.dsh-staging` **原样下发**，由执行器**在自己机器上展开**（服务器读不到另一台机器的用户目录，这是唯一正确的答案） |
| 拒绝要分开 | `occupied`（被别的账号占着，点名占用者）与 `no-executor`（没有执行器在线，给出补救）是两种不同的拒绝，混在一起会让人修错方向 |
| client | 徽标 + 按钮：未绑定→「本地模式」按钮；自己绑的→「解绑」；别人占的→只显示状态不给按钮；**没有共享规则→按钮禁用并说明原因** |

**实测**：

| 检查 | 结果 |
|---|---|
| `check-web-bind.mjs`（host 端点，真会话） | **12/12**：自动生成的可见路径、机器名、绑定→state 显示已绑定→解绑→回到未绑定；无会话 403；两种拒绝各自带上原因 |
| `check-execution-badge.mjs`（真浏览器） | **7/7**：控件出现在会话标题栏、说明当前执行位置、无共享规则时按钮禁用且给出理由、页面无脚本报错 |
| 服务端 `claim()` 的白名单 | `isClientAbsolutePath` 现在也接受 `%NAME%\…` 形式（**不展开**：这条路径属于客户端，展开必须发生在客户端） |

**如实说明没验到的**：**"点按钮成功绑定"这一条没能在浏览器里端到端跑过**。原因是探针 home 的工作区都落在共享规则**覆盖不到**的位置（`Desktop` 下的规则只覆盖一层子目录，而 `.dsh` 在它之外），所以 UI 上造不出"按钮可点"的会话；我加的第二条规则也没能命中。已验的是：端点本身 12/12、组件确实会把这个动作打到那个端点上（实测点击发出 `POST /client-web/bind`）。**上线实例上按钮是可点的**（它的工作区在 `C:\dsh-workspaces` 下，规则覆盖），所以这条留给线上首跑确认。
