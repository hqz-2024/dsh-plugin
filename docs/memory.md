...
4. **本地模式也能连上模型（2026-09-22 补）**：只烘服务器地址时，装完的客户端切到本地模式会**让你填 API key** —— 因为本地模式的模型面来自 `$DSH_HOME/settings.yaml` + `.credentials.yaml`，而那是 provisioning 才写的。现在打包把模型路由也烘进清单的 `dshDesktopGateway`（`scripts/desktop-gateway-environment.mjs`：origin 默认与服务器模式同源，`DSH_DESKTOP_GATEWAY_MODELS` 默认 flash/pro，`DSH_DESKTOP_GATEWAY_TOKEN` 可选），应用首启时 `src/desktop-gateway.ts` **只写缺失的那一个**：`settings.yaml` 与 `.credentials.yaml` 已存在就原样保留（手工/provisioning 配过的机器不被覆盖），凭据缺就补。`.env.windows` 里 `DSH_DESKTOP_GATEWAY=auto` + token 即"装完零配置"；token 留空则凭据不进安装包、每台机器各自跑 `provision-client.ps1`；`DSH_DESKTOP_GATEWAY=none` 整体关闭。**注意 token 是凭据**：它随安装包走（安装包只从设置页下载、不出局域网）。实测：全新 home 首启后 `settings.yaml` 落盘 → 用那份凭据打网关 `/llm/v1/models` 200、`/chat/completions` 200。
