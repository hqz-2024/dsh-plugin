...
- 客户端 profile：base + web-app + 本地工具 + `llm` 指向网关；**不带任何 key**，不给模型选择界面。
...
1. **本地会话同步形式**：形式 2「归档/文档」（建议 v1，无版本耦合）还是形式 1「服务器上可读的只读会话」（有版本锁 + 只读约束）。
2. **安装包体积**：核心包（约 100–200MB）还是全量包（含 ffmpeg 等，300–600MB）。
3. **外壳**：Electron（+150MB，最像豆包）／WebView2 宿主（体积小）／直接开浏览器（0 成本）。
4. **本地模式默认权限**：全权限，还是工作区写 + 审批。
