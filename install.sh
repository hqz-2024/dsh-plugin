...
# web = 全部命令在服务器执行；web-client = 已绑定的工作区由绑定它的机器执行。
PROFILE="__RUN_PROFILE__"
CADDY="${CADDY:-caddy}"
ROOT="$HOME/.dsh"
...
cd "$ENGINE_DIR"
pnpm dsh --profile "$PROFILE" --trusted-host "$LAN_IP"

kill "$CADDY_PID" 2>/dev/null || true
EOF
sed -i.bak "s|__LAN_IP__|$LAN_IP|; s|__ENGINE_DIR__|$ENGINE_DIR|; s|__RUN_PROFILE__|${RUN_PROFILE:-web}|" "$START"
rm -f "$START.bak"
chmod +x "$START"
ok "已生成启动脚本: $START"
