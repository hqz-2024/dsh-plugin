...
1. **拿包。** 在这台机器的浏览器里打开 `https://<服务器>:8443` 登录 → **设置 → 本地插件 → 「客户端执行器（executor）」那张卡片** → 点「**下载**」拿到 `dsh-executor.zip`（约 32 MB）。

2. **解压，双击 `dsh-executor.exe`。** 就这两步 —— 不需要命令行、不需要装 Node、不需要环境变量。

   双击之后它**自己会打开一个浏览器页面**（`http://127.0.0.1:38460`），那就是这个客户端的界面。在这个页面上依次：

   - **① 连接服务器**：填服务器地址（如 `https://192.168.28.239:8443`）与账号密码，点「登录并连接」。服务器会签发一枚**只属于这台电脑**的凭据（可在「设置 → 工作区绑定」里按机器撤销），之后重启自动连上，**不用再登录**。
   - **② 绑定工作区**：列表里点「绑定」。「本机可见路径」已经按服务器那边的共享规则**预填**好了（可改），已绑定的那行会显示「已绑定到这台电脑」并多出一个「解绑」。
   - **③ 工作区共享凭据**（可选但通常要填一次）：共享账号与密码，执行器存进本机凭据库，之后 `\\<服务器>\ws-<工作区>` 就像本地盘一样可用。
   - **④ 点「打开 Web UI」**：在浏览器里打开工作界面，开始正常工作。

   页面顶部一直显示**当前状态**（已连接 / 未连接 / 尚未配置，以及本机正在执行几个工作区），所以「没生效」这件事不用猜。

> **「打开 Web UI」第一次会让你登录一次。** Web UI 是**另一道门**：它认的是 dsh 进程自己的启动凭据，和执行器的凭据不是一回事。所以按钮给出的地址会先把浏览器带到登录页，登进去（那个账号如果提示绑定二次验证，可以跳过）之后才进工作界面，之后这个浏览器就一直记得。
>
> **看到 `dsh web authentication required; reopen the URL printed by dsh web` 怎么办**：说明拿到的是裸服务器地址而不是带凭据的入口 —— 通常是**执行器连不上服务器**时的回落（页面顶部会显示「未连接」）。先让它连上再点按钮。

3. **再装两个程序**（agent 的工具要用，装完不用再管）：
...
> **一定要解压，不要直接在压缩包里双击。** 包里有两样东西：`dsh-executor.exe` 和它旁边的 `node-pty\`。**终端（ConPTY）需要 `node-pty`**，原生模块没法打进 exe，所以它随包解压在那里，执行器启动时自己会找（用 `--self-test` 可确认）。只把 exe 单独拷走会**只丢交互式终端**，命令执行、HTTP 转发、文件读写都照常，并且终端第一次被请求时会报出确切原因。
...
> **`启动执行器.cmd`（一键启动脚本）仍然可以下载**，它是给"不想手填服务器和密码"的场景用的：脚本里写好了服务器地址、证书路径与一枚新签发的凭据，放进和 exe 同一个文件夹双击即可，之后就只差「绑定工作区」这一步。**它不是必需的** —— 走上面第 2 步的手工流程效果完全一样。

> **换新包之前先关掉旧的。** 正在运行的 exe 会占住文件（Windows 不允许删改运行中的 exe），要替换那个文件夹请先结束任务管理器里的 `dsh-executor`，再覆盖。
...
> **手动路径仍然可用**（想自己管一个 Node 环境时）：下载 `/dsh-subprocess-dispatch/executor.mjs`，自己设 `NODE_EXTRA_CA_CERTS` 指向 caddy 的根证书，然后 `node executor.mjs --server https://<服务器>:8443`，在配置页用账号登录一次再绑定；`npm i node-pty` 到它旁边就能得到终端，或用 `--node-pty <路径>` 指明。旧版执行器**不会**应答服务器的 ping，空闲时会被每约 9 秒判一次掉线并重连 —— 所以服务端升级后，客户端也要重新下载一次执行器（**症状**：那台机器的日志里反复出现 `disconnected — retrying in …`）。
