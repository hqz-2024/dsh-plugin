...
2. **解压，双击 `dsh-executor.exe`。** 就这两步 —— **不需要命令行、不需要装 Node、不需要填服务器地址、不需要登录、不需要任何参数。**

   服务器地址和**部署级机器密钥已经写在这个 exe 里**（构建时从服务器配置注入），所以双击之后它自己就连上了，并报出自己的机器标识（形如 `desktop-xxxx-1a2b3c4d`，第一次运行生成，之后不变）。它会打开一个本机状态页 `http://127.0.0.1:38460`，那里**只显示状态**：连上哪台服务器、本机标识、共享凭据是否已存。**没有工作区列表**（这是刻意的：该页面在本机环回上应答，同机任何进程都能访问）。
...
**工作区绑定在 Web UI 里做，不在执行器页面上**：打开左侧**文件树面板**，它的标题栏里有一个执行位置标签和按钮 —— 显示「服务器」表示该工作区的命令跑在服务器上，点「本地模式」即绑到**这台**电脑（显示「本地模式」+「解绑」按钮）。可见路径与暂存目录都由服务器按共享规则自动生成，**你一个字都不用填**。共享凭据仍在本机状态页填一次（只存本机，不发往服务器）。
